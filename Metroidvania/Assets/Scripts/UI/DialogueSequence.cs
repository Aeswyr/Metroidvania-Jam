...
public class DialogueSequence : MonoBehaviour
{
    ...
    private float floatTimeStamp;
    private float floatTimeStampRes;
    ...
    void Update()
    {
        ...
        {
            bubble2.transform.localPosition = bubble1.transform.localPosition + (Vector3)fadeDirection * fadeCurve.Evaluate((Time.time - floatTimeStamp) / fadeTime);
            float a = 1 - (Time.time - floatTimeStamp) / fadeTime;
        ...
        {
            fadeResponseBubble.transform.localPosition = responseBubbles[responseIndex].transform.localPosition + (Vector3)fadeDirection * fadeCurve.Evaluate((Time.time - floatTimeStampRes) / fadeTime);
            float a = 1 - (Time.time - floatTimeStampRes) / fadeTime;
    ...
    public void NextEvent()
    {
        ...
        if (index >= 0)
        {
            if (dialogueEvents[index].responses != null)
            {
